...
class Memory extends Test                                                       // Bit memory described by a layout.
 {Stack<Boolean> bits;                                                          // Bits comprising the memory with least significant bits stored lowest
  int at    = 0 ;                                                               // Position of memory in bits
  int width = 0;                                                                // Number of bits in memory

  Memory() {}                                                                   // Create a memory from another memory
  ...
  Memory(int Width)                                                             // Create a memory of specified width
   {width = Width;
    bits  = new Stack<>();
    for (int i = 0; i < width; ++i) bits.push(null);
   }
  ...
  static Memory memory(int size)                                                // Create memory of specified size
   {final Memory m = new Memory();
    m.bits = new Stack<>();
    for (int i = 0; i < size; i++) m.bits.push(false);
    m.width = size;
    return m;
   }
...
//D1 Layouts                                                                    // Layout memory as variables, arrays, structures, unions
    ...
    Layout layout()                                                             // Layout the structure based on the fields that describe it
     {fields.clear();                                                           // Clear of chain of fields in this layout
      ...
      bits = new Stack<>();                                                     // Memory for this layout
      for(int i = 0; i < width; i++) bits.push(false);                          // Initialize the memory to all false
